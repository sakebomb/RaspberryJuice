"""Entity descriptions and the classic numeric entity-type constants.

The ids are the classic mcpi/RaspberryJuice entity type ids used by ``spawnEntity`` and
the ``getEntities`` type filter. An ``Entity`` can be passed anywhere an id is needed.
"""
...
class Entity:
    """A minecraft entity type (id and optional name)."""

    def __init__(self, id: int, name: Optional[str] = None) -> None:
        self.id = id
        self.name = name

    def __eq__(self, rhs: object) -> bool:
        if not isinstance(rhs, Entity):
    ...
    def __iter__(self) -> Iterator[int]:
        """Allows an Entity to be sent whenever its id is needed."""
        return iter((self.id,))
